    ...
    return (
        <div className="rounded-xl border border-slate-200/90 bg-white p-6 sm:p-10 shadow-2xs">
            <div className="mx-auto max-w-2xl flex flex-col items-center">
                ...
                <Can permission="estudiantes.cargar">
                    <div
                        onClick={() => !isSubmitting && fileInputRef.current?.click()}
                        onDragOver={handleDragOver}
                        onDragLeave={handleDragLeave}
                        onDrop={handleDrop}
                        className={`group w-full rounded-2xl border-2 border-dashed p-8 sm:p-12 text-center transition-all cursor-pointer ${isDragOver
                ...
                <Can permission="estudiantes.agregar">
                    <button
                        type="button"
                        onClick={onAddManualClick}
                        className="inline-flex items-center gap-2 text-sm font-semibold text-slate-900 hover:text-[#002D62] transition-colors"
                    >
                ...
                <div className="mt-8 w-full rounded-xl border border-slate-200/80 bg-[#F4F6F8] p-5">
                    ...
                    <div className="font-mono text-xs text-slate-600 leading-relaxed bg-white/60 p-3 rounded-lg border border-slate-200/50 overflow-x-auto">
                    ...
                    <p className="mt-2 text-[11px] text-slate-600">
                        * Cada fila debe contener el nombre, apellido y código SIS único del estudiante.
                    </p>
                </div>
            </div>
        </div>
